...
def eval_policy(env, agent, n_trajectories, max_pathlength):
    # modify to compute discounted reward?
    r_ave = 0.0
    for _ in xrange(n_trajectories):
        obs = env.reset()
        for step in xrange(max_pathlength):
            action, action_dist = agent.act(obs)
            obs, r, done, _ = env.step(action)
            r_ave += r
            if done or step >= (max_pathlength-1):
                break

    return r_ave / n_trajectories
